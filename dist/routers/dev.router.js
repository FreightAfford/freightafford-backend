...
import { autoTransitSailedBookings, sendSailingReminders, } from "../jobs/sailing-notifications.js";
import Booking from "../models/booking.model.js";
import catchAsync from "../utils/catch-async.js";
const devRouter = Router();
devRouter.post("/test/auto-transit", catchAsync(async (req, res) => {
    await autoTransitSailedBookings();
    res
        .status(200)
        .json({ status: "success", message: "Job ran successfully" });
}));
devRouter.post("/test/sailing-reminder", catchAsync(async (req, res) => {
    await sendSailingReminders();
    res.status(200).json({ status: "success", message: "Reminders job ran." });
}));
devRouter.post("/test/monthly-report", catchAsync(async (req, res) => {
...
devRouter.get("/backfill/preview", catchAsync(async (req, res) => {
...
devRouter.post("/backfill/run", catchAsync(async (req, res) => {
    const bookings = await Booking.find({
        status: "confirmed",
        sailingDate: { $lt: new Date() },
    });
