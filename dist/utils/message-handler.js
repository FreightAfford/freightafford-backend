...
export const registerMessageHandlers = (io, socket) => {
    ...
    //
    socket.on(SOCKET_EVENTS.CHAT_MESSAGE, async ({ sessionId, content }) => {
        try {
            if (!content?.trim()) {
            ...
            const recipientIsInRoom = socketsInRoom.some((s) => s.data.user._id.toString() !== userId);
    ...
    //
    socket.on(SOCKET_EVENTS.CHAT_TYPING, ({ sessionId }) => {
        const timerKey = `${sessionId}:${userId}`;
        const roomName = ROOMS.session(sessionId);
    ...
    //
    socket.on(SOCKET_EVENTS.CHAT_STOP_TYPING, ({ sessionId }) => {
        const timerKey = `${sessionId}:${userId}`;
        const existingTimer = typingTimers.get(timerKey);
        if (existingTimer) {
            clearTimeout(existingTimer);
    ...
    //
    socket.on(SOCKET_EVENTS.CHAT_READ, async ({ sessionId }) => {
        try {
