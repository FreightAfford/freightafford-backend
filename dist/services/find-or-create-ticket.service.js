...
export const findOrCreateTicket = async (email) => {
    const extractedId = extractTicketId(email.subject);
    if (extractedId) {
        const existing = await Ticket.findOne({ ticket_id: extractedId });
        if (existing)
            return { ticket: existing, isNew: false, threaded: true };
    }
    return await createTicket(email);
};
export const touchTicket = async (ticketId) => await Ticket.findByIdAndUpdate(ticketId, {
    status: "open",
    last_message_at: new Date(),
});
//# sourceMappingURL=find-or-create-ticket.service.js.map