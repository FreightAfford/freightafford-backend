...
const MessageService = {
    ...
    sendMessage: async (payload) => {
        ...
        if (session.status !== "active") {
            throw new Error(`Cannot send messages to a session with status: ${session.status}`);
        }
        // Confirm the sender is a participant in this session
        const isCustomer = senderRole === "customer" && session.customer.toString() === senderId;
        const isCSO = senderRole === "cso" && session.assignedCSO?.toString() === senderId;
        if (!isCustomer && !isCSO) {
    ...
    //
    getMessages: async (sessionId, requesterId, requesterRole, page = 1, limit = 30) => {
        const session = await ChatSession.findById(sessionId);
        if (!session)
    ...
    //
    markAsDelivered: async (sessionId, recipientId) => {
    ...
    //
    markAsRead: async (sessionId, readerId) => {
        const result = await ChatMessage.updateMany({
            session: sessionId,
    ...
    //
    getUnreadCount: async (sessionId, userId) => {
    ...
    //
    getBulkUnreadCounts: async (sessionIds, userId) => {
