...
const ChatSessionService = {
    ...
    createSession: async (payload) => {
        ...
        return { session: populatedSession, queuePosition };
    },
    // ── Accept Session (CSO) ───────────────────────────────────────────────────
    //
    ...
    getAllSessions: async (filters, pagination) => {
        ...
        const [sessions, total] = await Promise.all([
            ChatSession.find(query)
                ...
                .populate("freightRequest", "originPort destinationPort status containerSize containerQuantity")
                .populate("booking", "bookingNumber vessel status sailingDate shippingLine"),
