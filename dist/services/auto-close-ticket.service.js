...
export const autoCloseStaleTickets = async () => {
    ...
    console.log(`[AutoClose] Found ${staleTickets.length} stale ticket(s). Closing...`);
    const results = await Promise.allSettled(staleTickets.map(async (ticket) => {
        await Ticket.findByIdAndUpdate(ticket._id, { status: "closed" });
        await sendTicketClosedNotification(ticket.customer_email, ticket.ticket_id, ticket.subject);
        console.log(`[AutoClose] Closed ticket #${ticket.ticket_id} for ${ticket.customer_email}`);
    }));
    const failed = results.filter((r) => r.status === "rejected");
    if (failed.length) {
        console.error(`[AutoClose] ${failed.length} ticket(s) failed to close:`, failed);
    }
};
//# sourceMappingURL=auto-close-ticket.service.js.map