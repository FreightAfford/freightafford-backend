...
export const initSocketServer = (httpServer) => {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on(SOCKET_EVENTS.CHAT_NEW_SESSION, async ({ sessionId }) => {
            try {
                if (isCSO)
                    return; // CSOs don't create sessions
                const session = await ChatSessionService.getSessionById(sessionId, userId, "customer");
