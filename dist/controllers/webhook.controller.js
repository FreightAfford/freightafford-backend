...
export const inboundWebhook = async (req, res, next) => {
    try {
        ...
        const { data, error } = await resend.emails.receiving.get(result.data.email_id);
        if (error)
            throw error;
        ...
        if (createdTicket.isNew) {
            if (!ticket.assigned_to)
                await assignCso(ticket._id);
            await sendTicketAcknowledgement(ticket.customer_email, ticket.ticket_id, ticket.subject);
        }
