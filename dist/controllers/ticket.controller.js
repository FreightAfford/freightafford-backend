...
export const replyTicketMessage = async (req, res, next) => {
    ...
    const reply = await replyToTicket(ticketId, req.user._id, req.user.fullname, message, files || []);
...
export const getSingleTicket = async (req, res, next) => {
    ...
    const ticket = await Ticket.findById(ticketId).populate("assigned_to", "fullname email role");
    if (!ticket)
