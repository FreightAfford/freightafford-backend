...
import {
  sendSailingReminderNotification,
...
export const autoTransitSailedBookings = async () => {
  ...
  const results = await Promise.allSettled(
    bookings.map(async (booking) => {
      ...
      if (error) {
        console.error(
          `[autoTransit] Email failed for booking ${booking.bookingNumber}: ${error}`,
        );
      }
      ...
      await createTrackingEvent({
        ...
        event: "in_transit",
        description:
          "Shipment automatically marked in transit on sailing date.",
        location: {
          originPort: (booking.freightRequest as any)?.originPort ?? "",
          destinationPort:
            (booking.freightRequest as any)?.destinationPort ?? "",
        },
        userId: "system",
      });
    }),
  ...
  console.error(
    `[autoTransit] Processed: ${bookings.length}, Failed: ${failed.length}`,
  );
};
...
export const sendSailingReminders = async () => {
  ...
  const results = await Promise.allSettled(
    bookings.map(async (booking) => {
      ...
      const { error } = await sendSailingReminderNotification(
        customer.email,
        customer.fullname,
        booking.bookingNumber,
      ...
      if (error) {
        console.error(
          `[sailingReminder] Email failed for booking ${booking.bookingNumber}:`,
          error,
        );
      }
    }),
  ...
  const failed = results.filter((r) => r.status === "rejected");
  console.log(
    `[sailingReminder] Reminders sent: ${bookings.length}, Failed: ${failed.length}`,
  );
};
