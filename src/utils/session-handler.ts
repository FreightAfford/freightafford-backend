...
export const registerSessionHandlers = (io: Server, socket: Socket): void => {
  ...
  socket.on(
    ...
    async ({ sessionId }: { sessionId: string }) => {
      try {
        ...
        const deliveredCount = await MessageService.markAsDelivered(
          sessionId,
          userId,
        );
  ...
  socket.on(
    ...
    async ({
      ...
      sessionId: string;
      action: "accept" | "close" | "reassign";
      toCSOId?: string; // required for reassign
    }) => {
      try {
        ...
        if (action === "accept") {
          // ── Accept ─────────────────────────────────────────────────────────
          const result = await ChatSessionService.acceptSession(
            sessionId,
            userId,
          );
          ...
          io.to(ROOMS.session(sessionId)).emit(
            SOCKET_EVENTS.CHAT_SESSION_UPDATED,
            {
              sessionId,
              status: "active",
          ...
          const result = await ChatSessionService.reassignSession(
            sessionId,
            userId,
            toCSOId,
          );
