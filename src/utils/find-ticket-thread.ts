...
export const findThreadTicket = async (email: any) => {
  ...
  if (inReplyTo) {
    ...
    if (oldMessage) return await Ticket.findById(oldMessage.ticket_id);
  }

  const subjectId = extractTicketId(email.subject);

  if (subjectId) return await Ticket.findOne({ ticket_id: subjectId });
