...
export const createAmendment = async (
  ...
  if (blockedStatuses.includes(booking.status))
    return next(
      new AppError(
        `Cannot submit amendment for ${booking.status} booking`,
        400,
      ),
    );
...
export const getAmendmentsByBooking = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const updateAmendmentStatus = async (
  ...
  if (!amendment) return next(new AppError("Amendment not found", 404));
...
export const getAmendmentById = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
