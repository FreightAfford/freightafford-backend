...
const QueueService = {
  assignPosition: async (sessionId: string): Promise<number> => {
  ...
  recalculateQueue: async (): Promise<QueueEntry[]> => {
    const waitingSessions = await ChatSession.find(
      { status: "waiting" },
      { _id: 1, customer: 1, createdAt: 1 },
    ).sort({ createdAt: 1 });
    ...
    if (bulkOps.length > 0) await ChatSession.bulkWrite(bulkOps);
  ...
  getQueue: async (): Promise<QueueEntry[]> => {
    const waitingSessions = await ChatSession.find(
      { status: "waiting" },
      { _id: 1, customer: 1, queuePosition: 1 },
    )
  ...
  getPositionForSession: async (sessionId: string): Promise<number | null> => {
    const session = await ChatSession.findById(sessionId, {
      status: 1,
      queuePosition: 1,
    });
